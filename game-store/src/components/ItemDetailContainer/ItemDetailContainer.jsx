...
const ItemDetailContainer = () =>{
    ...
    const {id} = useParams();
    const [itemJuego,setItemJuego] = useState(juego);
    const [flagContent, setFlagContent] = useState(false);
    ...
    const getItems = () =>{
        var aux = itemJuego.juegos.find(element => element.id ==id)
        setItemJuego(aux);
        setFlagContent(true);
    }
    ...
    useEffect(()=>{
        setTimeout(()=>{
            CargarJuego.then(getItems);
        },2000)       
    },[]);

    return flagContent? <ItemDetail juego={itemJuego}/>: false;
